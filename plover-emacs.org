...
* Opening bracket
[[http://www.openstenoproject.org/plover/][Plover]] is a libre software engine for stenography.  Stenography is a
fast (over 240 wpm), ergonomic, and flexible way to "type".  Although
invented before computers, stenography turns out to be a superb input
method for computing.  Plover converts a stroke of keys to some output
such as a character, word, or modified combination of these.  Stroke
mappings are defined within a dictionary file, typically JSON.  See
the Plover GitHub wiki entry "[[https://github.com/openstenoproject/plover/wiki/Dictionary-Format][Dictionary Format]]" for the dictionary
syntax.
...
simple as possible, while teaching users how to adjust the system to
