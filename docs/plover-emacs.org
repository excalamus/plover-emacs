...
for ={^ing}=.  Default alternate forms can preserve a single-stroke
form for some, like =HR-FPBG= for "listening" when using =HR-FPB= for
"listen" and =AO*E-PBG= for "Inc.".  Unfortunately, others must incur
one-stroke penalty of the alternate =/G= form.  The alternate is easy
to remember, so hopefully it's a fair compromise.
...
"Winning" requires a new single-stroke definition.
