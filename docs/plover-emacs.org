...
steno theory unifies a set of mappings within a cohesive conceptual
framework.  It is the stenographer's dictionary which captures the
essense of the theory.  A dictionary defines the rule(s) for a
mapping.  Stenographers have complete liberty to modify a theory
through updates to their dictionaries.  What follows are common
techniques used to design dictionaries and formulate theories.
