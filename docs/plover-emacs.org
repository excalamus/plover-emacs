...
If =KPW= or =-PBG= is taken as a 'base' shape, notice how Shift is
always the same key.  On the right hand side, the "Shift bit" is the
bottom left (=-R=).  Similarly, the "Super bit" is the top left
(=-F=).
