...
used only by court reporters and captioners.  The Open Steno Project,
however, provides software and resources free of charge for anyone
willing to learn.

#+caption: Georgi: Steno For All by [[https://www.gboards.ca/product/georgi][g Heavy Industries]]
file:./img/georgi.png

Dedicated steno machines and any keyboard which allows simultaneous
key presses can use Plover.  It works by converting a stroke of keys
to a character, word, phrase, or command.  Strokes, and the result
they produce, are defined within a dictionary file[fn:1].
...
essense of a theory.  What follows are common techniques used to
design dictionaries and formulate theories.
...
Emacs makes extensive use of modifier keys.  An old joke is that Emacs
stands for "Escape, Meta, Alt, Control, Shift".  This presents a
...
combination.  The shapes compose additively to define new shapes.
Notice how the keys used for [[control][Control]] and [[alt][Alt]] are the same keys used
for [[control-alt][Control + Alt.]]  The keys used for [[shift][Shift]] and [[alt][Alt]] are the same as
for [[alt-shift][Alt + Shift]].  And so on.  The shapes can be mirrored to the other
side of the board to yield more possible strokes.
...
A trick to help remembering how the shapes compose is to consider each
key as part or bit of the overall pattern.  Notice that the Shift
pattern always includes the same key mirrored appropriately for a
given side (right or left). On the right-hand-side, the "Shift bit" is
the lower left key (=-R=); on the left it's the lower right key (=R=).
...
apply and are included for completeness[fn:7].
...
[fn:7] Dictionary definitions for Super are not provided for at this
time.  Emacs doesn't commonly require Super and, because of the amount
of effort to define entries, its inclusion is considered low priority.
For anyone interested, see the notes explaining new strokes, alternate
strokes, and silly definitions.
...
[fn:5] Of course, it's possible to send 'µ' directly with Plover.
