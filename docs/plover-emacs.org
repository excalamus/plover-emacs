...
The last stroke, =UPBG= for "you think", is a common enough brief.
However, since that would map to control-e (end of the line), I'm
willing to take a stroke penalty (or come up with a different brief
for "you think"). A replacement for =O-PBG= ("-ing on") may be useful
but is not provided.
...
#+begin_quote
also - i've modified what my return (enter) stroke does specifically for use in the terminal
i make sure that my newline is attached on both sides
as without that, the next command you write will start with a space, and in bash won't be saved to your command history
that's also why i chose up to be the default direction for arrows...etc - to make scrolling through past commands easier :p

from my "emily-magnum.json" dictionary: "H-F": "{^}\n{^}",
"H-FPLT": "{^}.\n{^}{-|}",
#+end_quote
