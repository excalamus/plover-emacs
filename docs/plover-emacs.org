...
# todo explain C-A-S and then call it quits!
# Escape
...
distribute it, more or less, however.  It has been developed for more
than 40 years and, although modest in appearance, boasts countless
features and add-ons[fn:2].
...
'Alt+x' (notated =M-x=)[fn:4].  To change windows using the
...
characters or actions.  The mapping is often times one-to-many.  A
steno theory unifies a set of mappings by providing a cohesive
conceptual framework.  A dictionary defines the rule(s) for a
mapping. It is the stenographer's dictionary which captures the
essense of the theory.  Stenographers have complete liberty to modify
a theory through updates to their dictionaries.  What follows are
common techniques used to design dictionaries and formulate theories.

** TODO Sequence chains <<sequence_chains>>
