...
To remember how the shapes compose, consider each shape as constructed
by different "bits".  On the right-hand-side, the "Shift bit" is the
lower left key (=-R=); on the left it's the lower right key (=R=).
Similarly, the "Super bit" is top left (=-F=) or top right (=H=).
Which bits are present determine the pattern.
...
Patterns are combined with the [[right-hand-rule][Right Hand Rule]] to provide
comprehensive modifier coverage within the standard Plover theory.
Detailed descriptions of each modifier combination appear below.
...
dictionary as =TPEFBG=.  This form feels contorted and, given the
command's importance, should have a more accessible binding.
