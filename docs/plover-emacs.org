...
** Mode prefixes <<mode_prefix>>
A pseudo "mode" can be defined by prefixing all related commands with
the same pattern.  There are two ways to do this.  Either append keys
to a shape pattern (thus achieving single stroke) or define a
...
The [[http://www.openstenoproject.org/stenodict/dictionaries/cross_platform_movement.html][Cross Platform Movement]] dictionary by Ted Morin also uses a
single-stroke prefix.  In that sysetem, =STPH-= is the movement
prefix.  The =-R= key corresponds to "a" key of [[https://en.wikipedia.org/wiki/Arrow_keys#WASD_keys][WASD]], which in turn
corresponds to the left arrow key.  Therefore, =STPH-R= means "move
left".

** Right hand rule <<right_hand_rule>>
Observe that it's possible to write nearly the entire alphabet with
only the left hand.  All letters but "e" (=E=), "i" (=EU=), and "u"
(=U=) appear before =*= in the [[https://sites.google.com/site/learnplover/lesson-2-steno-order][steno order]].  In this way, the alphabet
is "leans left".

A consequence of the alphabet's tendency to lean left is:

#+begin_quote
*Right Hand Rule:* a single stroke for every letter in the alphabet
can be made by using a right hand pattern that excludes the =E= and
=U= keys.
#+end_quote
